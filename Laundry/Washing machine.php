...
// ตรวจสอบการจองเครื่องซักผ้า
if (isset($_POST['book_machine'])) {
    $machine_id = $_POST['machine_id'];

    // อัปเดตสถานะเครื่องซักผ้าเป็น 'ไม่ว่าง'
    ...
    // คืนค่าผลลัพธ์
    echo json_encode(['success' => true, 'machine_id' => $machine_id]);
    exit();
}
...
<div id="subtitle-wrapper" class="lazyload" data-bg="images/subtitle-wrapper01.jpg">
	<div class="subtitle-wrapper-img lazyload" data-bg="images/subtitle-wrapper01-img.jpg">
		<div class="container container-fluid-xl">
			...
			<h1 class="subtitle__title">Select Your Washing Machine</h1>
		</div>
    ...
    <div class="machine-list">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<img src='images/wwu machine.png' alt='Washing Machine {$row['machine_name']}'>";
...
<script>
function checkAndBook(machineId) {
    // ตรวจสอบสถานะของเครื่องซักผ้า
    fetch(`check_status.php?id=${machineId}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'ว่าง') {
                if (confirm(`เครื่องซักผ้า ${machineId} ว่างอยู่ คุณต้องการจองไหม?`)) {
                    // ทำการจองเครื่อง
                    bookMachine(machineId);
                }
            } else {
                alert(`เครื่องซักผ้า ${machineId} ไม่ว่าง`);
            }
        })
        .catch(error => console.error('Error:', error));
}

// ฟังก์ชันที่ตรวจสอบสถานะของเครื่องซักผ้าเป็นระยะ
function updateStatus() {
    const machineIds = [1, 2, 3, 4, 5, 6]; // ID ของเครื่องซักผ้า
    machineIds.forEach(machineId => {
        fetch(`check_status.php?id=${machineId}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById(`status-${machineId}`).innerText = data.status; // อัปเดตสถานะใน UI
            })
            .catch(error => console.error('Error:', error));
    });
}

// เรียกใช้ฟังก์ชัน updateStatus ทุก ๆ 5 วินาที
...
<script>
        function bookMachine(machineId) {
            if (confirm("คุณต้องการจองเครื่องซักผ้า " + machineId + " หรือไม่?")) {
                fetch('Washing machine.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: `book_machine=true&machine_id=${machineId}`
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        alert(`จองเครื่องซักผ้า ${data.machine_id} สำเร็จ`);
                        // อัปเดต UI เพื่อแสดงสถานะใหม่
                        location.reload(); // โหลดหน้าใหม่เพื่อแสดงสถานะที่อัปเดต
                    } else {
                        alert(`ไม่สามารถจองเครื่องซักผ้า ${machineId} ได้`);
                    }
                })
                .catch(error => console.error('Error:', error));
            }
